...
@Component({
  selector: 'checkout-cfd-cc',
  templateUrl: './checkout.html',
  styleUrls: ['./checkout.scss'],
  providers: [WebSocketService]
})

export class CheckoutCfdCardConnect implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  getSecretKey = (amount) => {
    ...
    this.configService.paymentAuth(params).subscribe((response: any) => {
      ...
      if (response.success === 1) {
        ...
        this.configService.captureOrder(paramsCapture).subscribe((captureResponse: any) => {
