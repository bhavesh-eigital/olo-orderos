...
@Component({
  selector: 'app-checkout-cfd-poynt',
  templateUrl: './checkout-cfd-poynt.component.html',
  styleUrls: ['./checkout-cfd-poynt.component.scss']
})
export class CheckoutCfdPoyntComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.poyntService.createPoyntScript("poyntWrapper", this.storeInformation, this.onReady, this.onGetNonce, this.onError);
  ...
  onGetNonce = ev => {
    if (ev?.data?.noce) {
      ...
      this.loading = true;
      this.getSecretKey(this.finalCurrentAmount, ev?.data?.noce);
    }
  };
  ...
  onError = (ev: any) => {
    ...
    if (ev?.data?.error?.source === 'submit') {
      this.openSnackBar(this.error, 2);
    }
    console.log(ev);
  };
  ...
  getSecretKey = (amount, nonce) => {
    ...
    this.configService.generatePoyntToken({
      ...
      switchMap((resp: any) => {
        console.log({poyntTokenResp: resp});
        if (resp.success === 1 && resp.token) {
          return this.configService.paymentAuth({
            amount,
            storeId: this.storeInformation.storeId,
            merchantId: this.storeInformation.merchantId,
      ...
      switchMap((response: any) => {
        ...
        if (response.success === 1) {
          return this.configService.captureOrder({
            storeId: this.storeInformation.storeId,
            merchantId: this.storeInformation.merchantId,
            amount,
            ...
            tap((captureResponse: any) => {
              if (captureResponse.success === 1) {
                this.generatePayment(response.data.id);
              } else {
  ...
  getToken(formData: any) {
    this.btnDisabled = formData.errorCode === '0' ? false : true;
