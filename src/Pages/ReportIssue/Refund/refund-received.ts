...
@Component({
  selector: 'refund',
  templateUrl: './refund-received.html',
  styleUrls: ['./refund-received.scss'],
})

export class RefundReceived implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.openSnackBar('Success! Your refund has been processed', 1);
  }
  ...
  getOrderPaymentsInformation = (storeInformation) => {
    this.configService.getOrderPaymentsRefund(storeInformation.merchantId, storeInformation.storeId, localStorage.getItem('orderId'))
      .subscribe((data: any) => {
        if (data.response.length === 0) {
          this.configService.getOrderPayments(storeInformation.merchantId, storeInformation.storeId, localStorage.getItem('orderId'))
            .subscribe((data2: any) => {
              this.paymentInfo = data2.response[0];
              this.tipSelected = this.paymentInfo.paymentTipAmount || 0;
